...
const FILE_TO_COLLECTION_MAP = {
  'exams.json': 'exams',
  'budgets.json': 'budgets',
  'blog.json': 'blog_posts',
  'support_labs.json': 'support_labs',
  'requisitions.json': 'requisitions',
  'professionals.json': 'professionals',
  'evaluations.json': 'evaluations',
  'eval_accesses.json': 'eval_accesses',
  'eval_hashes.json': 'eval_hashes',
  'message_templates.json': 'message_templates',
  'pops.json': 'pops',
  'non_conformities.json': 'non_conformities',
  'documents.json': 'documents',
  'transactions.json': 'transactions',
  'movements.json': 'movements',
  'pessoas.json': 'pessoas',
  'finance_settings.json': 'finance_settings',
  'access_profiles.json': 'access_profiles',
  'escala_plantao.json': 'escala_plantao',
  'cisnorpi.json': 'cisnorpi',
  'temperaturas.json': 'temperaturas',
  'cash_closures.json': 'cash_closures',
  'patients.json': 'patients',
  'appointments.json': 'appointments',
  'shortcuts.json': 'shortcuts',
  'convenios.json': 'convenios',
  'price_tables.json': 'price_tables',
  'recipientes.json': 'recipientes',
  'requisition_shortcuts.json': 'requisition_shortcuts',
  'materiais_coletados.json': 'materiais_coletados',
  'setores.json': 'setores',
  'locais_coleta.json': 'locais_coleta',
  'medicos.json': 'medicos',
  'lab_exames_alvaro.json': 'lab_exames_alvaro',
  'materiais_alvaro.json': 'materiais_alvaro',
  'lab_exames_pardini.json': 'lab_exames_pardini',
  'config_apoio_alvaro.json': 'config_apoio_alvaro',
  'config_apoio_pardini.json': 'config_apoio_pardini',
  'impressoras.json': 'impressoras',
  'interface_data.json': 'interface_data'
};

// Função helper para gravar arquivos JSON locais e sincronizar automaticamente no Firestore
function saveJsonFile(filePath, content, options) {
  ...
  if (db) {
    try {
      ...
      const collectionName = FILE_TO_COLLECTION_MAP[fileName];
      if (collectionName) {
        const parsedData = typeof content === 'string' ? JSON.parse(content) : content;
        syncToFirestore(collectionName, parsedData).catch(err => {
          console.error(`Erro ao sincronizar ${collectionName} com Firestore em saveJsonFile:`, err.message || err);
        });
      }
    } catch (err) {
      console.error(`Erro ao processar syncToFirestore em saveJsonFile:`, err.message || err);
    }
  }
}
...
const REQUISITION_SHORTCUTS_FILE = path.join(process.cwd(), 'data', 'requisition_shortcuts.json');
...
try {
  ...
  if (fs.existsSync(configPath)) {
    ...
    console.log(`🔥 Cloud Firestore ativado com sucesso! (Database: ${firebaseConfig.firestoreDatabaseId || '(default)'})`);
  } else {
    console.warn("⚠️ Arquivo firebase-applet-config.json não encontrado. Firestore não ativado.");
  }
} catch (fbInitErr) {
  console.error("Erro ao inicializar Firebase Admin / Firestore:", fbInitErr.message || fbInitErr);
}
...
async function initializeFirebaseCaches() {
  if (!db) {
    ...
    // Processar e garantir integridade das referências de convênio nas requisições e pacientes
    loadRequisitions();
    loadPatients();
    if (Array.isArray(requisitionsCache) && requisitionsCache.length > 0) {
      saveJsonFile(REQUISITIONS_FILE, JSON.stringify(requisitionsCache, null, 2), 'utf-8');
      saveCollectionToMysql('requisitions', requisitionsCache).catch(err => console.error("Erro ao salvar requisicoes na inicializacao:", err));
      syncToFirestore('requisitions', requisitionsCache);
    }
  ...
  try {
    console.log("🔥 Sincronizando e carregando todas as coleções do Firestore...");
    ...
    patientsCache = await loadCollectionFromFirestore('patients', PATIENTS_FILE);
    appointmentsCache = await loadCollectionFromFirestore('appointments', APPOINTMENTS_FILE);
    cashClosuresCache = await loadCollectionFromFirestore('cash_closures', CASH_CLOSURES_FILE);
    conveniosCache = await loadCollectionFromFirestore('convenios', CONVENIOS_FILE);
    priceTablesCache = await loadCollectionFromFirestore('price_tables', PRICE_TABLES_FILE);
    recipientesCache = await loadCollectionFromFirestore('recipientes', RECIPIENTES_FILE);
    materiaisColetadosMasterCache = await loadCollectionFromFirestore('materiais_coletados', MATERIAIS_COLETADOS_FILE);
    setoresCache = await loadCollectionFromFirestore('setores', SETORES_FILE);
    impressorasCache = await loadCollectionFromFirestore('impressoras', IMPRESSORAS_FILE);
    locaisColetaCache = await loadCollectionFromFirestore('locais_coleta', LOCAIS_COLETA_FILE);
    medicosCache = await loadCollectionFromFirestore('medicos', MEDICOS_FILE);
    transactionsCache = await loadCollectionFromFirestore('transactions', TRANSACTIONS_FILE);
    movementsCache = await loadCollectionFromFirestore('movements', MOVEMENTS_FILE);
    pessoasCache = await loadCollectionFromFirestore('pessoas', PESSOAS_FILE);
    accessProfilesCache = await loadCollectionFromFirestore('access_profiles', ACCESS_PROFILES_FILE);
    cisnorpiCache = await loadCollectionFromFirestore('cisnorpi', CISNORPI_FILE);
    temperaturasCache = await loadCollectionFromFirestore('temperaturas', TEMPERATURAS_FILE);
    nonConformitiesCache = await loadCollectionFromFirestore('non_conformities', NON_CONFORMITIES_FILE);
    escalaPlantaoCache = await loadCollectionFromFirestore('escala_plantao', ESCALA_PLANTAO_FILE);
    financeSettingsCache = await loadCollectionFromFirestore('finance_settings', FINANCE_SETTINGS_FILE);
    labExamesAlvaroCache = await loadCollectionFromFirestore('lab_exames_alvaro', LAB_EXAMES_ALVARO_FILE);
    materiaisAlvaroCache = await loadCollectionFromFirestore('materiais_alvaro', MATERIAIS_ALVARO_FILE);
    labExamesPardiniCache = await loadCollectionFromFirestore('lab_exames_pardini', LAB_EXAMES_PARDINI_FILE);
    configApoioAlvaroCache = await loadCollectionFromFirestore('config_apoio_alvaro', CONFIG_APOIO_ALVARO_FILE);
    configApoioPardiniCache = await loadCollectionFromFirestore('config_apoio_pardini', CONFIG_APOIO_PARDINI_FILE);
    interfaceCache = await loadCollectionFromFirestore('interface_data', INTERFACE_FILE);
    messageTemplatesCache = await loadCollectionFromFirestore('message_templates', MESSAGE_TEMPLATES_FILE);
    shortcutsCache = await loadCollectionFromFirestore('shortcuts', SHORTCUTS_FILE);

    // Garantir relacionamentos e integridade
    loadRequisitions();
    loadPatients();
    ...
    const hasOldCategories = Array.isArray(popsCache) && popsCache.some(p => p.category && !/^\d{2}\./.test(p.category));
    if (hasOldCategories) {
    ...
    if (Array.isArray(localPops)) {
      localPops.forEach(lp => {
        if (Array.isArray(popsCache) && !popsCache.some(p => p.id === lp.id)) {
          popsCache.push(lp);
          modifiedPops = true;
        }
      });
    ...
    if (Array.isArray(budgetsCache) && budgetsCache.length === 0 && Array.isArray(requisitionsCache) && requisitionsCache.length > 0) {
      const complexReqs = requisitionsCache.filter(r => r.exams && r.exams.length > 0);
      if (complexReqs.length > 0) {
    ...
    console.log("🔥 Todas as tabelas foram carregadas e sincronizadas com sucesso do Cloud Firestore!");
  } catch (error) {
...
async function cleanObsoleteDatabaseFields() {
  ...
  // 2. Garantir integridade de propriedades nos caches e arquivos locais
...
function loadRequisitions() {
  if (Array.isArray(requisitionsCache)) {
    ...
    requisitionsCache.forEach(r => {
      ...
      // 1. Relacionamento dinâmico com o Paciente (por patientCode, patientId, patientCpf ou patientName)
      let matchedPatient = null;
      if (r.patientCode) {
        matchedPatient = patientsList.find(p => String(p.code || p.id).trim().toLowerCase() === String(r.patientCode).trim().toLowerCase());
      }
      if (!matchedPatient && r.patientId) {
        matchedPatient = patientsList.find(p => String(p.id).trim().toLowerCase() === String(r.patientId).trim().toLowerCase());
      }
      if (!matchedPatient && r.patientCpf) {
        const cleanCpf = String(r.patientCpf).replace(/\D/g, '');
        if (cleanCpf) {
          matchedPatient = patientsList.find(p => String(p.cpf || '').replace(/\D/g, '') === cleanCpf);
        }
      }
      if (!matchedPatient && r.patientName) {
        const searchName = String(r.patientName).trim().toLowerCase();
        matchedPatient = patientsList.find(p => (p.name && p.name.trim().toLowerCase() === searchName) || (p.patientName && p.patientName.trim().toLowerCase() === searchName));
      }

      if (matchedPatient) {
        r.patientName = matchedPatient.name || matchedPatient.patientName || r.patientName || '';
        r.patientCode = matchedPatient.code || matchedPatient.id || r.patientCode || '';
        if (matchedPatient.cpf && !r.patientCpf) r.patientCpf = matchedPatient.cpf;
        if (matchedPatient.phone && !r.patientPhone) r.patientPhone = matchedPatient.phone;
        if (matchedPatient.birthDate && !r.patientBirthDate) r.patientBirthDate = matchedPatient.birthDate;
        if (matchedPatient.gender || matchedPatient.sex) r.patientSex = matchedPatient.sex || matchedPatient.gender || r.patientSex;
      }

      // 2. Relacionamento dinâmico com o Médico Solicitante (por doctorId, doctorCode, doctorCrm ou doctorName)
      let matchedDoctor = null;
      if (r.doctorId) {
        matchedDoctor = medicosList.find(m => String(m.id).trim().toLowerCase() === String(r.doctorId).trim().toLowerCase());
      }
      if (!matchedDoctor && r.doctorCode) {
        matchedDoctor = medicosList.find(m => String(m.codigo || m.id).trim().toLowerCase() === String(r.doctorCode).trim().toLowerCase());
      }
      if (!matchedDoctor && r.doctorCrm) {
        const cleanCrm = String(r.doctorCrm).replace(/\D/g, '');
        const ufMatch = r.doctorUf ? String(r.doctorUf).trim().toLowerCase() : null;
        if (cleanCrm) {
      ...
      if (!matchedDoctor && r.doctorName) {
        const searchDoc = String(r.doctorName).trim().toLowerCase().replace(/^dr\.?\s*/i, '').replace(/^dra\.?\s*/i, '');
        if (searchDoc) {
          matchedDoctor = medicosList.find(m => {
            const mName = String(m.nome || m.name || '').trim().toLowerCase().replace(/^dr\.?\s*/i, '').replace(/^dra\.?\s*/i, '');
            return mName && mName === searchDoc;
          });
        }
      }
      if (!matchedDoctor && (!r.doctorName || !r.doctorCrm) && medicosList.length > 0) {
        matchedDoctor = medicosList[0];
      }

      if (matchedDoctor) {
        r.doctorName = matchedDoctor.nome || matchedDoctor.name || r.doctorName || '';
        r.doctorCrm = matchedDoctor.numero || matchedDoctor.crm || r.doctorCrm || '';
        r.doctorUf = matchedDoctor.uf || r.doctorUf || 'PR';
        r.doctorCrmType = matchedDoctor.conselho || r.doctorCrmType || 'CRM';
        if (!r.doctorId) r.doctorId = matchedDoctor.id;
        if (!r.doctorCode) r.doctorCode = matchedDoctor.codigo || matchedDoctor.id;
      }

      // 3. Garantir que o campo de convênio esteja sempre preenchido e vinculado
      let matchedConv = null;
      if (r.convenioId) {
        matchedConv = conveniosList.find(c => String(c.id) === String(r.convenioId));
      }
      if (!matchedConv && r.convenioCode) {
        matchedConv = conveniosList.find(c => String(c.codigo) === String(r.convenioCode) || String(c.id) === String(r.convenioCode));
      }
      if (!matchedConv && (r.convenioCode || r.convenio)) {
        const search = String(r.convenioCode || r.convenio).toLowerCase().trim();
        matchedConv = conveniosList.find(c => (c.fantasia && c.fantasia.toLowerCase().trim() === search) || (c.razaoSocial && c.razaoSocial.toLowerCase().trim() === search));
      }
      if (matchedConv) {
        r.convenio = matchedConv.fantasia || matchedConv.razaoSocial;
        if (!r.convenioCode && matchedConv.codigo) r.convenioCode = matchedConv.codigo;
        if (!r.convenioId && matchedConv.id) r.convenioId = matchedConv.id;
      } else if (r.convenioCode && r.convenioCode.length > 2 && isNaN(Number(r.convenioCode))) {
        r.convenio = r.convenioCode;
      } else if (!r.convenio) {
        r.convenio = 'Particular';
      }

      // Normalizar status da requisição conforme regra: Pronto, Cancelado ou Em Andamento
      const stStr = String(r.status || '').trim();
      if (stStr.toLowerCase().includes('cancel')) {
        r.status = 'Cancelado';
      } else if (Array.isArray(r.exams) && r.exams.length > 0 && r.exams.every(e => ['Conferido', 'Liberado', 'Pronto', 'Concluído'].includes(String(e.status || '').trim()))) {
        r.status = 'Pronto';
      } else {
...
function updateRequisitionCombinedExams(r) {
  if (!r || typeof r !== 'object') return;
  if (!Array.isArray(r.exams)) {
    r.exams = [];
  }

  const examItems = r.exams.map((ex, idx) => {
    const code = ex.code || ex.codigo || `EXAME_${idx + 1}`;
    const name = ex.name || ex.titulo || ex.tituloLaudo || code;
    const status = ex.status || 'Pendente';
    const result = ex.result !== undefined ? ex.result : (ex.resultado !== undefined ? ex.resultado : '');
    const unit = ex.unit || ex.unidade || '';
    const ref = ex.referenceValue || ex.valorReferencia || '';
    const method = ex.method || ex.metodo || '';
    const material = ex.material || '';
    const equipment = ex.equipment || ex.equipamento || '';
    const obs = ex.observations || ex.observacoes || '';
    const interp = ex.interpretation || ex.interpretacao || '';
    const typedBy = ex.typedBy ? ` (Digitado por: ${ex.typedBy}${ex.typedAt ? ' em ' + ex.typedAt : ''})` : '';
    const conferidoBy = ex.conferidoBy ? ` (Conferido por: ${ex.conferidoBy}${ex.conferidoAt ? ' em ' + ex.conferidoAt : ''})` : '';
    const liberadoBy = ex.liberadoBy ? ` (Liberado por: ${ex.liberadoBy}${ex.liberadoAt ? ' em ' + ex.liberadoAt : ''})` : '';

    let textBlock = `[EXAME ${idx + 1}: ${code} - ${name}]
Status: ${status}${typedBy}${conferidoBy}${liberadoBy}
Resultado: ${result ? result : 'Sem resultado'} ${unit}`.trim();

    if (ref) textBlock += `\nValor de Referência: ${ref}`;
    if (method) textBlock += `\nMétodo: ${method}`;
    if (material) textBlock += `\nMaterial: ${material}`;
    if (equipment) textBlock += `\nEquipamento: ${equipment}`;
    if (obs) textBlock += `\nObservações: ${obs}`;
  ...
  const fullTextSummary = examItems.map(item => item.textBlock).join('\n\n----------------------------------------\n\n');

  r.examesJuntos = fullTextSummary;
  r.examesTexto = fullTextSummary;
  r.examesConcatenados = JSON.stringify(examItems);
  r.examesCompletos = examItems;
}

function saveRequisitions(requisitions) {
  try {
    if (Array.isArray(requisitions)) {
      requisitions.forEach(r => updateRequisitionCombinedExams(r));
    }
...
const tableSchemas = {
  ...
  exame_materiais_coletados: [
    ...
    { name: 'materialCodigo', type: 'VARCHAR(100)' }, // Código relacional do material
    { name: 'material', type: 'VARCHAR(255)' }, // Nome / Descrição do material
...
let requisitionShortcutsCache = null;

function loadRequisitionShortcuts() {
  if (!requisitionShortcutsCache) {
    requisitionShortcutsCache = loadLocalJson(REQUISITION_SHORTCUTS_FILE);
  }
  const exams = loadExams() || [];
  return (requisitionShortcutsCache || []).map(s => {
    if (s.examId) {
      const found = exams.find(e => String(e.id || e.code) === String(s.examId) || String(e.code) === String(s.examCode));
      if (found) {
        s.examCode = found.code || found.jalisCode || s.examCode || '';
        s.examName = found.name || s.examName || '';
      }
    }
    return s;
  });
...
function saveRequisitionShortcuts(shortcuts) {
  try {
    ...
    saveJsonFile(REQUISITION_SHORTCUTS_FILE, JSON.stringify(shortcuts, null, 2), 'utf-8');
    ...
    syncToFirestore('requisition_shortcuts', shortcuts);
  } catch (error) {
    console.error("Erro ao salvar atalhos de requisição:", error);
  }
}
    ...
    if (Array.isArray(exams)) {
      ...
      exams.forEach(ex => {
        ...
        matList.forEach((m, idx) => {
          const matCodigo = m.materialCodigo || m.codigo || m.materialId || (m.material ? String(m.material).trim() : '');
          relationalMateriais.push({
            id: `${ex.id || ex.code}_${matCodigo || m.abrev || idx}`,
            examId: ex.id || ex.code || '',
            examCode: ex.code || ex.jalisCode || '',
            materialCodigo: matCodigo,
            material: m.nome || m.materialNome || m.material || '',
            abrev: m.abrev || '',
            prazo: m.prazo || '',
            metodo: m.metodo || '',
            amb: m.amb || '',
            ...
            ordem: m.ordem || '',
            copias: m.copias || 0,
            proibir: m.proibir ? 1 : 0,
            naoImprimir: m.naoImprimir ? 1 : 0,
            proibidoApoio: m.proibidoApoio ? 1 : 0,
            paginaSeparada: m.paginaSeparada ? 1 : 0,
            coleta: m.coleta || '',
            preparo: m.preparo || '',
            amostra: m.amostra || '',
            inadequada: m.inadequada || '',
            referencia: m.referencia || '',
            procedimento: m.procedimento || '',
            conservacao: m.conservacao || '',
            mensagem: m.mensagem || '',
            planilha: m.planilha || '',
            sorotecaDias: m.sorotecaDias || 0,
            sorotecaUnidade: m.sorotecaUnidade || '',
            sorotecaConservacao: m.sorotecaConservacao || '',
            recipientesColeta: m.recipientesColeta || [],
            recipientesTriagem: m.recipientesTriagem || [],
            insumos: m.insumos || [],
            taxas: m.taxas || [],
            custoSimples: m.custoSimples || 0,
            precoCusto: m.precoCusto || 0,
            precoBase: m.precoBase || 0,
            precoFator: m.precoFator || 1,
            precoCh: m.precoCh || 0,
            precoFilme: m.precoFilme || 0,
            priceTableValues: m.priceTableValues || [],
            modeloLaudo: m.modeloLaudo || '',
            observacoesLaudo: m.observacoesLaudo || '',
            tituloLaudo: m.tituloLaudo || '',
            materialLaudo: m.materialLaudo || '',
            metodoLaudo: m.metodoLaudo || '',
            valorReferenciaLaudo: m.valorReferenciaLaudo || '',
            lisLayouts: m.lisLayouts || {},
            medInterferentes: m.medInterferentes || '',
            medSuspender: m.medSuspender || '',
            conveniosRegras: m.conveniosRegras || [],
            setorBancada: m.setorBancada || '',
            loinc: m.loinc || '',
            unidadeMedida: m.unidadeMedida || '',
            validadeLaudo: m.validadeLaudo || '',
            labApoio: m.labApoio || '',
            codigoApoio: m.codigoApoio || '',
            tempTransporte: m.tempTransporte || '',
            recipienteTransporte: m.recipienteTransporte || '',
            labExternoList: m.labExternoList || [],
            codigoHl7: m.codigoHl7 || '',
...
function renderPdfFormattedText(doc, text, options = {}) {
  ...
  lines.forEach((line) => {
    ...
    if (validTokens.length > 0) {
      doc.text('', { continued: false }); // fecha linha
    }
  });
}
...
function safeReadImageBuffer(input) {
  ...
  try {
    ...
    // Se o buffer foi corrompido com a marca UTF-8 efbfbd (0xEF 0xBF 0xBD substituindo 0x89)
...
function drawFooter(doc, pageNum, totalPages, sigInfo) {
  ...
  if (marcaDaguaBuf) {
    try {
      ...
      // Imagem PNG corrompida ou incompatível, usará o fallback vetorial abaixo
    }
  }
...
async function generatePdfForRequisition(reqFound) {
...
app.all(['/api/paciente/laudo/pdf', '/api/pacientes/laudo/pdf', '/api/laudo/pdf'], async (req, res) => {
  try {
    ...
    const hasTypedResults = (reqFound.exams || []).some(e => 
      Boolean(String(e.resultado || e.result || e.resultadoText || '').trim()) || 
      (Array.isArray(e.linhas) && e.linhas.length > 0)
    );
    const isLiberado = ['liberado', 'concluido', 'pronto', 'conferido', 'laudado', 'digitado', 'coletado'].includes(statusStr) ||
                      (reqFound.exams || []).some(e => ['liberado', 'conferido', 'pronto', 'concluido', 'laudado', 'digitado'].includes(String(e.status || e.situacao || '').toLowerCase())) ||
                      hasTypedResults;
    ...
    const forceRegenerate = req.query?.force === 'true' || req.query?.force === '1' || req.query?.reload === 'true';

    let pdfBase64 = reqFound.pdfBase64 || reqFound.laudoPdfBase64;

    if (!pdfBase64 || forceRegenerate) {
...
function loadPatients() {
  ...
  if (Array.isArray(patientsCache)) {
    const conveniosList = loadConvenios();
    patientsCache.forEach(patient => {
      if (!patient.convenio || typeof patient.convenio !== 'string' || patient.convenio.trim() === '') {
        let matched = null;
        if (patient.convenioId) {
          matched = conveniosList.find(c => String(c.id) === String(patient.convenioId));
        }
        if (!matched && patient.convenioCode) {
          matched = conveniosList.find(c => String(c.codigo) === String(patient.convenioCode) || String(c.id) === String(patient.convenioCode));
        }
        if (matched) {
          patient.convenio = matched.fantasia || matched.razaoSocial;
        } else if (patient.convenioCode) {
          patient.convenio = patient.convenioCode;
        } else {
...
app.get('/admin/resultados/pdf-status', requireAdmin, (req, res) => {
  try {
...
app.post('/admin/resultados/salvar-exame', requireAdmin, async (req, res) => {
  try {
    ...
    // Marca requisição como 'gerando PDF' para controle de status
...
const saveRequisitionHandler = (req, res) => {
  try {
    ...
    const reqData = {
      ...
      status: (() => {
        const inputSt = (status && status.trim()) ? status.trim() : 'Em Andamento';
        if (inputSt.toLowerCase().includes('cancel')) return 'Cancelado';
        if (Array.isArray(parsedExams) && parsedExams.length > 0 && parsedExams.every(e => ['Conferido', 'Liberado', 'Pronto', 'Concluído'].includes(String(e.status || '').trim()))) {
          return 'Pronto';
        }
        return 'Em Andamento';
      })(),
...
app.post('/admin/requisicoes/cancel', requireAdmin, handleCancelRequisition);
app.post('/admin/requisicoes/cancel-ajax', requireAdmin, handleCancelRequisition);
app.post('/admin/requisicoes/delete', requireAdmin, handleCancelRequisition);
app.post('/admin/requisicoes/delete-ajax', requireAdmin, handleCancelRequisition);

// Atualizar Pagamento / Baixa no Caixa
app.post('/admin/requisicoes/update-payment', requireAdmin, (req, res) => {
  try {
    ...
    importedExams.forEach(item => {
      ...
      if (existingIndex !== -1) {
        ...
        currentExams[existingIndex] = {
          ...
          fasting: String(item.fasting || existingExam.fasting || 'Não obrigatório').trim(),
          timeframe: String(item.timeframe || existingExam.timeframe || '24 horas').trim(),
          instructions: String(item.instructions || existingExam.instructions || '').trim(),
...
app.post('/admin/atalhos-requisicao/add', requireAdmin, (req, res) => {
  try {
...
app.post('/admin/atalhos-requisicao/edit', requireAdmin, (req, res) => {
  try {
...
app.post('/admin/atalhos-requisicao/delete', requireAdmin, (req, res) => {
  try {
    const { id } = req.body;
    if (!id) {
      return res.redirect('/admin/atalhos-requisicao?error=not_found');
...
function startServer() {
  ...
  (async () => {
    try {
      ...
      console.error("Erro ao inicializar caches:", err);
    }
    try {
